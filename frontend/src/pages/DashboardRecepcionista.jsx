...
export function DashboardRecepcionista() {
  ...
  const citasPendientes = todasLasCitas.filter(c => c.estado === 'AGENDADA' || c.estado === 'MODIFICADA');
  const citasCompletadas = todasLasCitas.filter(c => c.estado === 'COMPLETADA');
  const citasCanceladas = todasLasCitas.filter(c => c.estado === 'CANCELADA');
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '2rem', fontWeight: 700, color: '#2563eb' }}>{todasLasCitas.length}</div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontWeight: 600, marginRight: '0.5rem' }}>Filtrar por estado:</span>
      ...
      <h2 style={{ marginBottom: '1rem' }}>
        {modoBusqueda ? 'Resultados de Busqueda' : 'Todas las Citas'}
        <span style={{ color: '#6b7280', fontSize: '0.875rem', fontWeight: 400, marginLeft: '0.5rem' }}>
